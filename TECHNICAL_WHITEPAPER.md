...
8. **DP 노이즈 (Stage 6):** 영양소별 민감도 보정된 라플라스 노이즈 주입 (ε=0.5/쿼리)
...
5. 이미 섭취한 양 차감
6. 의료 제약 적용 (엄격한 경계 — `/engine/medical-constraints` API)
7. 남은 예산을 시간 버킷에 분배
8. NutrientBudget 출력 (완전한 감사 추적 포함)
```
...
### Stage 6: 차분 프라이버시 노이즈 주입

**파일:** `engine/pipeline.py` — `_stage_dp_noise()`

**목적:** 파이프라인 출력에 수학적으로 보정된 라플라스 노이즈를 적용하여 ε-차분 프라이버시를 보장합니다.
...
| 파라미터 | 값 | 설명 |
|---|---|---|
| $\epsilon$ | 0.5/쿼리 | 전체 예산 $\epsilon_{total}$ = 1.0, 24시간 리셋 |
| $\Delta_n$ | 영양소별 | kcal: 500, carbs: 50, water: 500 등 |

**안전 보장:**
- 노이즈 적용 값은 `[minimum, maximum]` 범위로 클램핑 (음수 영양소나 상한 초과 방지)
- 프라이버시 예산 소진 시 → 원본 값 유지 (안전 장치)
- 감사 추적: `dp_noise:applied` 또는 `dp_noise:skipped`
...
수학적으로 보정된 노이즈가 집계 데이터에 추가됩니다:

- **라플라스 메커니즘:** 수치 쿼리 (평균 혈당, 총 걸음 수)
- **가우시안 메커니즘:** (ε, δ)-DP를 통한 고차원 쿼리
- **프라이버시 예산:** 사용자당 $\epsilon_{total}$ = 1.0, 24시간 리셋
- **파이프라인 통합:** Stage 6에서 쿼리당 $\epsilon$ = 0.5로 모든 영양소 타겟에 라플라스 노이즈 적용
...
| `privacy/` | 4 | ~1,200 | 프라이버시 보호 계층 |
| `services/` | 2+ | ~700 | FHIR 임포터, 분석기 |
| `routers/` | 6 | ~900 | API 엔드포인트 (의료 제약 포함) |
| `tests/` | 3 | ~2,100 | 111개 통과 테스트 (동기화, 엔진, 특허 갭, 자가 교정) |
| **합계** | **27+** | **~10,100** | — |
